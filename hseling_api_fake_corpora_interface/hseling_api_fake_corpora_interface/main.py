...
from hseling_api_fake_corpora_interface import boilerplate

from hseling_lib_fake_corpora_interface.process import process_data
from hseling_lib_fake_corpora_interface.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
app.config.from_envvar('HSELING_API_FAKE_CORPORA_INTERFACE_SETTINGS')
app.config.update(
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_fake_corpora_interface.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "Application Fake Corpora Interface"})
